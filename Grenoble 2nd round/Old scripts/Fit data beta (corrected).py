...
rad=np.pi/180
inf_file_name="beta_alpi8off_gamma_chicoarse_BETA_02Jul1638"
sorted_fold_path="/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 2nd round/exp_CRG-3033/Sorted data/"+inf_file_name
correct_fold_path="/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 2nd round/exp_CRG-3033/Corrected data/"+inf_file_name
beta_fold_clean=correct_fold_path+"/Beta/AlphaON"
...
fit_res0=[173.16963573, 157.91070318, 48.78914678, -0.64436549]
err_res0=[0.4308915, 0.58322297, 0.05915978, 0.00372778]
max0=fit_res0[-1]
for i in range(len(ps_pos)):
    matrix[i]=tot_data[:,1][tot_data[:,-1]==ps_pos[i]]/tot_data[:,1][tot_data[:,-1]==ps_pos[i]]
for i in range(len(ps_pos)):
    P0=[5, np.amax(matrix[i]), 48,-0.6]
    # print(P0)
    ...
    ax.errorbar(c_pos,matrix[i],yerr=np.sqrt(matrix[i])/5,fmt="ko",capsize=5)
...
P0=[(np.amax(ps_data)+np.amin(ps_data))/2, np.amax(ps_data)-np.amin(ps_data), 3,ps_pos[0]+0.5]
...
ax[0].errorbar((ps_pos-ps_pos[0]-p1[-1])*w_ps/np.pi,-beta, yerr=err_b,fmt="ko",capsize=5)
