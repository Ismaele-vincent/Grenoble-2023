...
def exp_w1p(x):
    return alpha*(1/(1+a21*np.exp(-1j*x))).real
...
inf_file_name="beta_alpi8off_gamma_chicoarse_GAMMA_01Jul1712"
...
for i in range(len(ps_pos)):
    matrix[i]=tot_data[:,2][tot_data[:,-1]==ps_pos[i]]/tot_data[:,3][tot_data[:,-1]==ps_pos[i]]
...
for i in range(len(coil)):
    ps_data=matrix[:,i]
    P0=[(np.amax(ps_data)+np.amin(ps_data))/2, np.amax(ps_data)-np.amin(ps_data), 2.6,ps_pos[0]*2.6]
    B0=([0,0,0,-10],[np.inf,np.inf,np.inf,np.inf])
    p,cov=fit(fit_cos,ps_pos,ps_data, p0=P0, bounds=B0)
    x_plt = np.linspace(ps_pos[0], ps_pos[-1],100)
    # fig = plt.figure(figsize=(5,5))
    # ax = fig.add_subplot(111)
    # ax.errorbar(ps_pos,ps_data,yerr=matrix_err[:,i],fmt="ko",capsize=5)  
...
fig = plt.figure(figsize=(5,5))
ax = fig.add_subplot(111)
ax.errorbar(coil,c_data,yerr=np.sqrt(c_data)/10,fmt="ko",capsize=5)  
ax.plot(x_plt,fit_cos(x_plt, *p), "b")
ax.vlines(p[-1]/p[-2],0,fit_cos(p[-1]/p[-2], *p),ls="dashed")
w_c=p[-2]
print("w_c=",w_c)
