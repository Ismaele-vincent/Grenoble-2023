...
P0=[(np.amax(ps_data)+np.amin(ps_data))/2, np.amax(ps_data)-np.amin(ps_data), 1,ps_pos[0]*1]
...
c_data_err=np.sum(matrix_err,axis=0)
...
fig = plt.figure(figsize=(5,5))
ax = fig.add_subplot(111)
ax.errorbar(coil,c_data,yerr=c_data_err,fmt="ko",capsize=5)  
ax.plot(x_plt,fit_cos(x_plt, *p), "b")
ax.vlines(p[-1]/p[-2],0,fit_cos(p[-1]/p[-2], *p),ls="dashed")
w_c=p[-2]
print("w_c=",w_c)
