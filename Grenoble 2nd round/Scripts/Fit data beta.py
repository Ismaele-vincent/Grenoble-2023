...
rad=np.pi/180
inf_file_name="beta_alpi8off_chi_26Jun1028ff"
...
for i in range(len(ps_pos)):
    ...
    # print(P0)
    p0,cov0=fit(fit_cos,coil,matrix[i], p0=P0, sigma=np.sqrt(matrix[i]))
...
P0=[(np.amax(ps_data)+np.amin(ps_data))/2, np.amax(ps_data)-np.amin(ps_data), 3,0.5]
B0=([0,0,0,-10],[np.inf,np.inf,np.inf,np.inf])
...
for i in range(len(ps_pos)):
    ...
    B0=([0,0,0,-10],[np.inf,np.inf,np.inf,np.inf])
    # print(P0)
    p,cov=fit(fit_cos,coil,matrix[i], p0=P0, sigma=np.sqrt(matrix[i]))
    err=np.diag(cov)**0.5
    # print(p[3], err[3])
    x_plt = np.linspace(coil[0], coil[-1],100)
    w[i]=p[2]
