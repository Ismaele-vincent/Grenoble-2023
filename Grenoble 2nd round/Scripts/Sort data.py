...
# inf_files_alpha=["S2Z+_17Jun1652","S2Z-_17Jun1600"]
# inf_path_alpha = [fold_path+inf_files_alpha[0]+".inf",fold_path+inf_files_alpha[1]+".inf"]
# inf_file_name="S2Z+-_17Jun1652"
# sorted_fold_path="/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 2nd round/exp_CRG-3033/Sorted data/"+inf_file_name
# if not os.path.exists(sorted_fold_path):
#     os.makedirs(sorted_fold_path)
...
# alpha_fold_raw=rawdata+"/Alpha"
# alpha_fold_clean=cleandata+"/Alpha"
# if (not os.path.exists(alpha_fold_raw)) or (not os.path.exists(alpha_fold_clean)):
#     os.makedirs(alpha_fold_raw)
#     os.makedirs(alpha_fold_clean)

# inf_files = np.genfromtxt(inf_path, dtype=str,usecols=(0,2))
# inf_files_alpha = np.genfromtxt(inf_path_alpha[0], dtype=str,usecols=(0))
# inf_files_alpha = np.append(inf_files_alpha,np.genfromtxt(inf_path_alpha[1], dtype=str,usecols=(0)))

# for root, dirs, files in os.walk(fold_path, topdown=False):

#     i=0
#     for name in files:
#         if (name in inf_files_alpha):
#             print(name)
#             shutil.copy(os.path.join(root, name), alpha_fold_raw+"/"+name)
#             cleantxt=np.loadtxt(os.path.join(root, name), skiprows=25, comments="*", delimiter="\t")[:,1:]
#             with open(alpha_fold_clean+"/"+name, 'w') as f:
...
#         #         np.savetxt(f, cleantxt,  header= "p