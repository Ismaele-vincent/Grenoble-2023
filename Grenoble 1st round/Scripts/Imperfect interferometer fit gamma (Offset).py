...
def fit_I_px(x, gamma0, chi0, w_c, w_ps, C, eta, A):
    ...
    fit_I_px = (I_px_co(0, chi, C, alpha, gamma) + eta/4*A*(0.5+a1*a2*np.cos(chi)) + I_px_in(0, chi, eta, alpha, gamma))
    # print(fit_I_px)
    return fit_I_px.ravel()

P0 = (c_0, ps_0, w_c, w_ps, 1.5, 2e-1, 0)
B0 = ([0, 0, 1, 7, 0, 0, -5], [1000, 1000, 5, 10, np.inf, np.inf, 10])
p, cov = fit(fit_I_px, range(len(matrix.ravel())),
             matrix.ravel()/np.amax(matrix.ravel()), bounds=B0)
print(p)
fig = plt.figure(figsize=(5, 5))
ax = fig.add_subplot(111)
...
def I_px(gamma0, chi0, w_c, w_ps, C, eta, A):
    gamma = w_c*c_amp-gamma0
    chi = w_ps*ps_pos-chi0
    gamma, chi = np.meshgrid(gamma, chi)
    fit_I_px = I_px_co(0, chi, C, alpha, gamma) +  eta/4*A*(0.5+a1*a2*np.cos(chi))+I_px_in(0, chi, eta, alpha, gamma)
...
def I_px_new(gamma0, chi0, w_c, w_ps, C, eta, A):
    gamma = w_c*c_amp-gamma0
    chi = w_ps*ps_pos-chi0
    gamma, chi = np.meshgrid(gamma, chi)
    fit_I_px = 2*I_px_co(0, chi, C, alpha, gamma)
    # print(fit_I_px)
    return fit_I_px
...
gamma, chi = np.meshgrid(gamma, chi)
Z = matrix
Z1 = I_px(*p)*np.amax(matrix)
Z2=I_px_new(*p)*np.amax(matrix)
...
# ax.contour3D(gamma, chi, Z2, 40, cmap='plasma')
ax.set_xlabel('$\\gamma$')
ax.set_ylabel('$\chi$')
ax.set_zlabel('z')
ax.view_init(40, 45)
...
for i in range(len(ps_pos)):
    ...
    with open(correct_fold_path+"/gamma_ps_"+str("%02d" % (i,))+".txt", 'w') as f:
        np.savetxt(f, np.transpose(data_txt),  header= "Current(A) O-Beam err ps_pos", fmt='%.7f %.7f %.7f %.7f' )
