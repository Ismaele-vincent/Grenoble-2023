...
def fit_I_px(x, gamma0, chi0, w_c, w_ps, C, eta, A):
    ...
    for i in range(len(chi)):
        if i==0:
            fit_I_px = I_px_co(beta[i], chi[i], C, alpha, gamma) + A+ I_px_in(beta[i], chi[i], eta, alpha, gamma)
        else:
            fit_I_px = np.vstack((fit_I_px, I_px_co(beta[i], chi[i], C, alpha, gamma) +A+ I_px_in(beta[i], chi[i], eta, alpha, gamma)))
    # print(fit_I_px)
    return fit_I_px.ravel()
